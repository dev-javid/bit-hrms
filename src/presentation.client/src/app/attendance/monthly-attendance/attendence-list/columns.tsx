...
export const getColumns = (
  onRegularizeClick: (inoutTiming: InOutTiming) => void
) => {
  const columns: ColumnDef<InOutTiming>[] = [
        ...
        return (
          <div className="">
            {row.original['status'] == 'RegularizationRequested' ? (
              <div className="flex space-x-2">
                {timeDifference(
                  row.original['regularization']!.clockInTime,
                  row.original['regularization']!.clockOutTime
                )}
                <RegularizationIcon
                  tooltip="Calculated based on regularization."
                  reason={''}
                />
              </div>
            ) : (
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return row.original['status'] == 'RegularizationRequested' &&
          !row.original['clockInTime'] ? (
          <div className="flex space-x-2 text-blue-700 font-medium">
            ...
            <RegularizationIcon
              tooltip="Missed clock in."
              reason={row.original['regularization']!.reason}
            />
          </div>
        ) : (
          <div className="text-blue-700 font-medium">
            {row.original['clockInTime']
              ? convertUtcTimeToLocalTime(row.original['clockInTime'])
              : ''}
          </div>
        );
      },
        ...
        return (
          <div className="text-blue-700 font-medium">
            {row.original['status'] == 'RegularizationRequested' ? (
              ...
              <>
                {row.original['clockOutTime']
                  ? convertUtcTimeToLocalTime(row.original['clockOutTime'])
                  : ''}
              </>
            )}
          </div>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <div className="flex space-x-2">
            {row.original['status'] == 'ClockInMissing' ||
            row.original['status'] == 'ClockOutMissing' ? (
              <Button
                variant="outline"
                onClick={() => onRegularizeClick(row.original)}
              >
                Regularize
              </Button>
            ) : null}
