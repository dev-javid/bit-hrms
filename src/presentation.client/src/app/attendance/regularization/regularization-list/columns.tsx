...
export const getColumns = (
  onApproveClick: (regularization: AttendanceRegularization) => void,
  user: User
) => {
  const columns: ColumnDef<AttendanceRegularization>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        return (
          <div className="space-x-2 text-blue-700 font-medium">
  ...
  if (user.isCompanyAdmin) {
    columns.push({
      accessorKey: 'action',
      header: ({ column }) => (
        <DataTableColumnHeader column={column} title="Approve" />
      ),
      cell: ({ row }) => {
        return (
          <div className="flex space-x-2">
            {!row.original['approved'] && (
              <Button
                variant="outline"
                onClick={() => onApproveClick(row['original'])}
              >
                Approve
              </Button>
            )}
