using Application.Incomes.Commands;

namespace Tests.Integration.Tests.Incomes;

public class AddIncome(TestWebApplicationFactory<Program> factory) : IntegrationTest(factory)
{
    [Fact]
    public async Task Should_add_new_income()
    {
        await LoginAsCompanyAdminAsync();
        await FeedDataAsync("Tests/Incomes/AddIncome.sql");

        var command = new AddIncomeCommand
        {
            IncomeSourceId = 1,
            Amount = 100,
            Documents = ["data:image/png;base64,",
                         "data:image/png;base64,"
            ],
            Remarks = Faker.Lorem.Sentence(),
        };

        HttpResponseMessage response;
        using (StringContent content = new StringContent(JsonSerializer.Serialize(command), Encoding.UTF8, JsoneMediaType))
        {
            response = await Client.PostAsync("/api/incomes", content);
        }

        response.StatusCode.Should().Be(HttpStatusCode.OK);
    }
}
